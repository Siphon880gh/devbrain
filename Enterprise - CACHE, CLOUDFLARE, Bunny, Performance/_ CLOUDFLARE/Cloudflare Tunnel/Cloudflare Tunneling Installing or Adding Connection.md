...
Instead of sending traffic directly to those ports, the `cloudflared` daemon creates outbound-only connections from your server to Cloudflare. Your app is then reached through Cloudflare, usually on a subdomain or an url path, without exposing the actual port to the public internet.
...
Although Cloudflare tunnel can connect path and/or subdomain, this guide will just do subdomain.

You _can_ serve something like:
```
- app1.example.com/app that delivers content from port 5000
- app1.example.com/ that delivers content from port 5000
- example.com/app that delivers content from port 5000
```
...
if __name__ == "__main__":
    app.run(debug=True)
    # NO # app.run(host="0.0.0.0", port=5000, debug=True)
```