...
It's helpful to have either this combination of SFTP users:
- Less secured but more flexibility:
	- SFTP user based on website name (what you will be doing most the time)
	- Root user
- Most secured:
	- SFTP user based on website name. Root user is passwordless so it'll be all SSH terminal, no easy Filezilla access

To setup root SSH user, refer to: [[Users - Create root user]]
If you're using Cloudpanel, each site user is the SSH/SFTP user for that website (only has access to that website's root files)

1. Collect your SSH credentials
2. Setup in Filezilla according to the type of SSH user that will act as SFTP. Recommend creating both root and non-root in Filezilla. Non-root makes sure pages can be viewed and that web browser visited scripts can modify/create files as intended.
