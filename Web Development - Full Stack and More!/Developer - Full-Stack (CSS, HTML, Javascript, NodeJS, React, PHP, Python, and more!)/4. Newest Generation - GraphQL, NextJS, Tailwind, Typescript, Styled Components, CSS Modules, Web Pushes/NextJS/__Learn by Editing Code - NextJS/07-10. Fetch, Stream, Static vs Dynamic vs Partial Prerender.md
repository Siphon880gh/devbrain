...
So all three fetches will be performed in parallel. It’ll only take as long as the slowest fetch. However, it’s an awkward white screen until all fetches are ready. If on a slow computer, it would be like this (slowed down to demonstrate):
...
Instead of streaming with the words “Loading” with a spinner before the rest of the fetched content renders, you can have a skeleton of the layout! It’s a more modern trend with loading sprites.  
