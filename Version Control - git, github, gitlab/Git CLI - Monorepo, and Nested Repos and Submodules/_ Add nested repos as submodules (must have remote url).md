...
Now, add the nested repository as a submodule. You'll need the remote URL you checked earlier. Run the following command, again replacing `path/to/submodule` with the path and `https://example.com/repo.git` with the actual remote URL:  
...
Btw the `git submodule add` command with URL will make sure there's file contents in the nested folder. If there isn't, it will automatically pull from the Github repo.
