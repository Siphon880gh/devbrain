...
**Meaning:** Traffic sent to your machine (host) on `HOST_PORT` is forwarded to the app listening inside the container on `CONTAINER_PORT`.

_Example:_  
`-p 8080:80` maps port `8080` on your host to port `80` inside the container.

If a web server runs on port `80` inside the container:

```
curl http://localhost:8080
```

Or a POST request to an API endpoint:

```
curl -X POST http://localhost:8080/api/users \
  -H "Content-Type: application/json" \
  -d '{"name":"Weng","role":"admin"}'
```

Inside Docker’s internal network, other containers would still access it on the container port:
...
When entering commands, your terminal prompt represents your **host machine**, e.g.:

```
(base) wengffung@Wengs-MacBook-Pro-New push %
```

Think of the syntax direction like this:

```
user → host : container
```

✔ The **left side** of `:` is always closer to you (the host).  
✔ The **right side** is inside Docker (the container).
...
Mentally read it as:
> “From my machine → into the container.”
