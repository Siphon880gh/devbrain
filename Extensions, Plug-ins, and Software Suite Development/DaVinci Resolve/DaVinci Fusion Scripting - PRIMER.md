...
Note if you're using the DaVinci Resolve API to load the .comp fusion script into a clip's fusion composition, that method `clip.ImportFusionComp(fusion_path)` is not supported in .py files in free verison. You have to manually type that into the DaVinci console (Workspace -> Console). You could have multiple lines of python code there (make sure to select Python 3 instead of having it on the language LUA) even though it appears the input is single line. That has been an inconsistency since 2020 and still a problem as of this writing 2024 Sept per [https://www.steakunderwater.com/wesuckless/viewtopic.php?t=4317&start=15](https://www.steakunderwater.com/wesuckless/viewtopic.php?t=4317&start=15).
