...
You have to **account for race conditions** when one part of your Cursor AI prompt **edits a file** and another part **produces an output that depends on that file** EVEN if they're two separate sentences following one after another.
...
- Generate a **CSV file** that refers to document lines (often with line numbers) and a summary of change for those lines, so you can reference exact locations later.
    
Opus 4.5 Agent Mode does not exactly have common sense when you given those two tasks. You have to be explicit that one task is done before the next task is run. Opus 4.5 may randomly find it efficient to perform both tasks simultaneously in some runs and in other runs, it may perform the tasks consecutively like the sentences are ordered.
...
- Cursor AI may start generating the CSV before all inline comments are inserted on some runs.
