...
- Make SSH login passwordless SSH authentication ([[SSH - Passwordless Key-Based Authentication Setup]])
- Implement Fail2Ban ([[_PRIMER - Fail2Ban]])
...
- Manually inspect `/tmp`, `/var/tmp`, and `/dev/shm` for scripts or binaries.
- Check for reverse shells or cron jobs:
```
crontab -l
sudo ls /etc/cron*
```
...
Scanners:
- Refer to [[Scanners (Rootkits and Malware)]]